...
export const getPostMovies = (imdbID) => {
  return function (dispatch) {
    ...
    axios
      ...
      .then((res) => res.data)
      .then((data) => {
        postMovies = [...postMovies, data, imdbID];
...
export const getCorusel = () => {
  return async (dispatch) => {
    try {
      const searchLine = "Matrix";
      const res = await axios.get(
        baseUrl + `?s=${searchLine}&apikey=${apiKey}`
      );
      const data = res.data.Search;
      dispatch({ payload: data, type: ADD_CORUSEL });
    } catch (error) {
      if (!error) {
        throw console.log("Error");
      }
